...
def scrapePolitico():
    ...
    #this removes the suggested articles sidebar so we don't scrape it.
    for element in soup.find_all('div', {'class':'widget-content'}):
        element.decompose()
...
#Scrapes LATimes articles.
#Added by Alec Gilbert August 2018
def scrapeLATimes():
    #This author writes most of the pieces about the supreme court for the LATimes.
    supreme_court_link = 'http://www.latimes.com/la-bio-david-savage-staff.html'
    #downloads page in soup format
    soup = grabPage(supreme_court_link)
    #each scraper function is different because of the differing formats.
    articles = soup.find_all('h5', {'class' : ''})
    articles += soup.find_all('a', {'class' : 'recommender'})

    second_link = 'http://www.latimes.com/search/?q=supreme+court'
    soup = grabPage(second_link)
    articles += soup.find_all('div', {'class' : 'h7'})
    
    
    urls = []
    
    #For loop extracts links from the h5 tags and a class="recommender" tags.
    for article in articles:
        path = article.get('href')
        if path==None:
            link = article.find('a')
            path = link.get('href')
        
        if path!=None:
            urls.append("http://www.latimes.com" + path)
    return urls
