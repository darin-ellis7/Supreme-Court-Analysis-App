...
#This is the new version from September 2018
...
import siteScraperFunctions as parser
...
def download_image(url,idImage):
    try:
        path = "/var/www/html/cs499SupremeCourt/webapp/images/" # for this to work the images folder must be in the same directory as this script
...
# Updated slightly by Alec
def relevant(keywords, title, source):

    justices = ['Ruth Bader Ginsberg','Anthony Kennedy','Chief Justice John Roberts','Clarence Thomas','Stephen Breyer','Elena Kagan','Samuel Alito','Sonya Sotomayor','Neil Gorsuch','Brett Kavanaugh']
    ...
    stateAbbreviations = ["AL", "AK", "AZ", "AR", "CA", "CO", "CT", "DC", "DE", "FL", "GA", 
                          ...
                          ,"Wyo.", "British", "Ala.", "Ariz.", "Ky.", "UK", "U.K."]
    #I added some that I happened to see in usage. Not all of them states

    #The full names of Justices in the title mean automatic acceptance.
    supremeCourt = ["John Roberts", "Anthony Kennedy", "Clarence Thomas",
    ...
    for judge in supremeCourt:
        if "justice " + judge.lower() in title:
            return True
    ...
    if 'supreme court' in title:
        ...
        for abv in stateAbbreviations:
            #This was changed to avoid some false negatives
            if (abv + ' supreme court' in capsTitle or 
				abv+' Supreme Court' in capsTitle):
                split = title.split()
...
def specialParsing(a, source)
    if(source=="latimes"):
        return parser.parseLATimes(a)
    if(source=="jdsupra"):
        return parser.parseJDSUPRA(a)
    if(source=="nytimes"):
        return parser.parseNYTimes(a)
    if(source=="chicagotribune"):
        return parser.parseChiTribune(a)
    ...
    for post in feed.entries:
        ...
        if ArticleIsDuplicate(title,c):
            ...
            try:
                ...
                text=""
                if source in ["latimes","jdsupra","nytimes","chicagotribune"]:
                    text = specialParsing(a,source)
                else:
                    text = a.text    
...
def main():
    # connect to database
    db = MySQLdb.connect(host=os.environ['DB_HOST'],port=int(os.environ['DB_HOST']),user=os.environ['DB_USER'],password=os.environ['DB_PASSWORD'],db="SupremeCourtApp",use_unicode=True,charset="utf8")
    db.autocommit(True)
